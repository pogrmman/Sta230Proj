...
protests <- protests %>% filter(!is.na(size)) %>%  bind_rows(non_numeric) %>%
  mutate(size = 10^(floor(log10(size))))
```

### Standarize for Population
```{r}
...
protests <- protests %>% left_join(populations, by=c("country" = "Country.Name",
                                                     "year" = "Year")) %>%
  mutate(size = as.numeric(size)) %>% mutate(popPct = (size / Population) * 100)
```